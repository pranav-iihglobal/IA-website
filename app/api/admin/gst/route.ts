...
import { currentPeriod, parsePeriod } from "@/lib/erp/gst-period";
import {
  b2bCsv,
  b2csCsv,
  buildGstReturn,
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // ?period=2026-09 as the pages write it, or year and month; the current
    // month in IST otherwise — the same default the screen shows.
    const today = currentPeriod();
    const fromPeriod = parsePeriod(params.get("period"));
    const year = fromPeriod?.year ?? (Number(params.get("year")) || today.year);
    const month = fromPeriod?.month ?? (Number(params.get("month")) || today.month);
